...
K_AFFECTED_CHANNELS = 10  # fixed for ALL conditions and BOTH systems
...
# ---------------------------------------------------------------------
def generate_factorial_dataset(case_fn, seed, n_normal=3000, n_per_condition=750,
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Verifying per-channel magnitude matching (independent vs correlated)")
    print("=" * 70)
    verify_magnitude_matching()
    ...
    for system_name, case_fn in [("14-bus", pn.case14), ("118-bus", pn.case118)]:
        print("\n" + "=" * 70)
        print(f"Factorial experiment: IEEE {system_name}  (k={K_AFFECTED_CHANNELS} fixed)")
        print("=" * 70)

        results = []
        for seed in SEEDS:
            print(f"  Seed {seed}...", end=" ", flush=True)
            X, y, n_failed = generate_factorial_dataset(case_fn, seed=seed)
            r = evaluate_seed(X, y, seed)
            results.append(r)
            print(f"F1(indep_low)={r['f1_independent_low']:.3f}  "
        ...
        df.to_csv(f"fdia_datasets/factorial/{system_name}_factorial_results.csv", index=False)
        all_results[system_name] = df

        print(f"\n  --- {system_name} Aggregate (mean +/- std) ---")
        ...
        all_tests[system_name] = run_factorial_tests(df, system_name)

    with open("fdia_datasets/factorial/factorial_summary.json", "w") as f:
        json.dump(all_tests, f, indent=2)

    print("\n" + "=" * 70)
    print("Done. Per-seed CSVs and factorial_summary.json saved to")
    print("fdia_datasets/factorial/. Send the console output back for")
    print("integration into the manuscript (Section 4.4, H2 test).")
    print("=" * 70)
